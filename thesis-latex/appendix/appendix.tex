...
\begin{lstlisting}[style=java, label={lst:user-supplied-externals-main}, caption={Solving an ASP program with custom external atoms.}]
...
Having parsed all XML elements, the next step is to determine which of these are terminal elements in the sense that they only contain text, as opposed to elements which consist of further XML elements. This is achieved using the code from Listing~\ref{lst:results-xml-parse-element-types}.
