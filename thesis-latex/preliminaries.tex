...
\todo{define positive and negative rule body as sets with distinct notation!}
...
\todo{Since we're talking about nonground stratification, should we define a nonground $T_P$ operator! Juust simply say we add the rules where all defining rules have already fired in each step}
...
Let $P$ be an ASP program. Then the \emph{dependency graph}  $\mathit{DG}_P = (R, D)$ is a directed graph with vertex set $R$, which has one element for each rule in $P$, and (dependency-)edge set $D$ such that
\begin{align*}
