...
\paragraph{Generalizing list-unwrapping}
...
As we can observe in Listings~\ref{lst:results-xml-graphcol-userinput} and~\ref{lst:results-xml-graphcol-write-output}, respectively, reading the content of a file, as well as writing to a file, requires a set of rules that looks the same regardless of the specific file or content in question. Listing~\ref{lst:discussion-fileio-readlines} shows the "standard program" to read all lines from a file:
\begin{itemize}
	\item A file path (i.e. location in storage), represented using a fact of form \texttt{infile(PATH)}, is opened.
...
Recalling the demand for \emph{action transparency} (see Definition~\ref{def:evolog-actions-transparency}), i.e. that the result of every firing (ground) action rule must be part of an answer set, modules containing actions can not simple be used in module literals in the way they are described in Section~\ref{sec:evolog-modules}. Instead, we would have to apply the definitions of action functions in rule heads to an Evolog Module and have a module function as an action interpretation function. Example~\ref{ex:discussion-fileio-actmodules} demonstrates this approach.
...
\end{lstlisting}
In the program shown in Listing ~\ref{lst:discussion-asp-q-universal-translated}, the first rule encodes the unversally quntified program $P$ using an Evolog module $modP$. The second rule then passes each termified answer set $AS$ of $modP$ into another module program $modP$ which encodes the program $C$ of the original ASP(Q) program. Finally, the constraint in the last line of Listing~\ref{lst:discussion-asp-q-universal-translated} ensures the program only has an answer set if, for each answer set $AS$ of $P$, there is also an answer set for $AS \cup C$.

While we have suggested simple encodings for ASP(Q) programs with a single quantifier using (nonground) Evolog, it is harder to draw a proper comparison between ASP(Q) and Evolog in terms of expressiveness. ASP(Q) takes disjunctive, ground, \gls{asp} programs as a starting point and adds the capability to reason over answer sets of such programs in the same language, thereby making ASP(Q) suitable to express any problem in the polynomial hierarchy. Evolog modules on the other hand are conceptually oracle functions for problems of the same computational complexity as can be expressed in a module-free Evolog program (if we don't allow modules to call further modules). In the simplest case, if we assume all Evolog programs to be ground and disjunction-free (and therefore able to express problems in $NP$), it would mean that problems with evaluation complexity in $\sigma^2_P$ can be expressed using ground Evolog programs with (non-nested) modules. Based on the complexity results on modular logic programs from~\cite{modular-lp-complexity}, we can conclude that nesting Evolog modules would allow one to express problems on any level of the polynomial hierarchy. In this case, the nesting depth would correspond to the level in the polynomial hierarchy one can express. Summing up, in the ground and function-free case, ASP(Q) is more expressive due to its unrestricted numbers of nested quantifiers. To reach the same levels of expressiveness in Evolog, we would need to allow modules to be nested.