...
% Recalling the representation of action result terms (see Section~\ref{subsucsec:implementation-actions-execution} and Definition~\ref{def:action-rule-syntax}) as function terms $r(v)$ with function symbol $r~\in~\{\mathit{success},~\mathit{error}\}$, any module-based abstraction of actions would have to make sure to represent the result of executing all actions in the sub-program as one atom (which gets translated to a term in module interpretation) representing either success or failure of the complete program. Special care needs to be taken in case of errors where some actions have already succeeded before, e.g. when writing the 31st of 50 lines to a file fails, but 30 lines habe already been written.