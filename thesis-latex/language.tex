...
Let $r_{app}$ be a non-ground anction application rule with head $H = a_{res}(f_{act}, S, I, f_{act}(S, I))$, $F$ a frame, and $H_{grnd} = a_{res}(f_{act}, S_{grnd}, I_{grnd}, r)$ a ground instance of $H$ with $r$ being an arbitrary ground term.
...
A herbrand interpretation $I$ of an Evolog Program is an \emph{Evolog Model} ("answer set") of an Evolog program $P$ with respect to a frame $F$ if and only if it is a minimal classical model of its Evolog-Reduct $P_{F}^{A}$.
\end{definition}
...
TODO: Find good (source for and) definition of (model-theoretic) conservative extension and add here!

\begin{theorem}
\label{thm:extension}
Every ASP program is a valid and semantically equivalent Evolog program.
\end{theorem}

\begin{proof}
Directly from the definitions of an Evolog program \todo{put somewhere "an Evolog program is an ASP program with actions and modules"} and the Evolog Reduct (see Definition \ref{def:evolog-reduct}):
\begin{itemize}
	\item Since an Evolog program is an ASP program that may additionally use action rules and module literals, it follows that every ASP program is an Evolog program which does not use actions or modules, and therefore a valid Evolog program.
	\item Equivalence between Evolog- and Stable Model Semantics for programs without actions and modules similarly follows from Definition \ref{def:evolog-reduct}: Since a regular ASP program does not contain any action rules, its Evolog Reduct coincides with its GL-Reduct. \todo{also reference module semantics here!}
\end{itemize}
\end{proof}