...
\end{itemize}
\end{definition}
In accordance with Definition \ref{def:action-function}, an action witness $action\_result(r_1, fileInputStream, PATH, OP\_RES)$ then reads as "Function $fileInputStream$, with action source $r_1$, applied to input $PATH$, given world state $(PATH)$, gives result $OP\_RES$". \todo{this should be done nicer. Actually, a function is applied to its input, given a rule body holds, that's exactly the elegance, we wanna put this into math-speak.}
